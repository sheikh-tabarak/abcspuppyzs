...
function loadCards() {
    for (let index = 0; index < Litters.length; index++) {
        ...
        if (Litters[index].mode === 0) {
            document.getElementById('litter-container').innerHTML += '<div class="w-full max-w-screen-xl mx-auto shadow m-8 py-10  border-[1px] rounded flex flex-wrap justify-center items-center  lg:divide-y-0 lg:divide-x-2"> <div class="w-[100%] md:w-[100%] lg:w-[70%] lg:px-10"> <div class="text-2xl font-extrabold px-10 lg:px-0 text-center lg:text-left ">' + Litters[index].name + '</div>  <table class="py-4 w-full mt-2"> <thead> </thead><tbody class="">  <tr class="even:bg-white odd:bg-gray-50  border-b border-t "> <th scope="row" class="px-4 py-2 w-[40%] lg:w-[35%] font-bold text-gray-900 text-right "> Litter Type: </th> <td class="px-4 py-2 text-left">' + Litters[index].type + ' </td> </tr> <tr class="even:bg-white odd:bg-gray-50  border-b "> <th scope="row" class="px-4 py-2 font-bold text-gray-900 text-right "> Average Weight: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].avg_weight + ' </td> </tr> <tr class="even:bg-white odd:bg-gray-50  border-b "> <th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Litter Status: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].status.text + '</td> </tr>         <tr id="expected-or-born' + index + '" class="even:bg-white odd:bg-gray-50  border-b "> </tr>      <tr id="age-' + index + '" class="even:bg-white odd:bg-gray-50  border-b "> </tr>             <tr id="go-home-row' + index + '" class="even:bg-white odd:bg-gray-50  border-b ">   </tr></tbody><tbody id="prices-container' + index + '"></tbody>  </table>  <div class="px-10 pt-4 flex justify-center" id="thumbnail-container-' + index + '"></div> </div> <div class="w-[100%] lg:w-[30%] px-5"> <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-300"> <li class="py-3 sm:py-4"> <div class="flex flex-wrap justify-center lg:justify-start items-center"> <div class="relative"> <div onclick="showMainPopup(' + index + ')" class="absolute flex justify-center items-center right-10 top-10 cursor-pointer"> <div class=" absolute w-10 h-10 bg-white  rounded-full animate-ping"> </div> <div class="absolute"><svg class="hover:scale-[1.1] transition duration-1000" width="40px" height="40px" viewBox="0 0 24 24" fill="gray" xmlns="http://www.w3.org/2000/svg"> <rect width="24" height="24" fill="none" /> <path fill-rule="evenodd" clip-rule="evenodd" d="M7 12C7 11.4477 7.44772 11 8 11H8.01C8.56228 11 9.01 11.4477 9.01 12C9.01 12.5523 8.56228 13 8.01 13H8C7.44772 13 7 12.5523 7 12ZM11 12C11 11.4477 11.4477 11 12 11H12.01C12.5623 11 13.01 11.4477 13.01 12C13.01 12.5523 12.5623 13 12.01 13H12C11.4477 13 11 12.5523 11 12ZM16 11C15.4477 11 15 11.4477 15 12C15 12.5523 15.4477 13 16 13H16.01C16.5623 13 17.01 12.5523 17.01 12C17.01 11.4477 16.5623 11 16.01 11H16ZM2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12Z" fill="white" /> </svg> </div> </div> <img class="w-96 rounded-lg" src="' + Litters[index].image + '" alt="Neil image"> </div> </li> </ul> </div> </div>';


            for (let inner_index = 0; inner_index < Litters[index].prices.length; inner_index++) {
                if (Litters[index].prices[inner_index].price != "") {
                    document.getElementById('prices-container' + index).innerHTML += '<tr class="even:bg-white odd:bg-gray-50  border-b border-t"> <th scope="row" class="px-4 py-2 font-bold text-gray-900 text-right "> <span class="text-wrap pl-2" id="availability-badge-' + index + inner_index + '"></span> ' + Litters[index].prices[inner_index].title + ' </th> <td class="px-4 py-2 text-left">' + Litters[index].prices[inner_index].price + '</td> </tr>'
            ...
            if (Litters[index].status.mode != 'available') {
                document.getElementById('go-home-row' + index).innerHTML = '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Go-Home Weekend: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].goHome + ' </td>'
            }


            if (Litters[index].status.mode == 'expecting') {

                document.getElementById('expected-or-born' + index).innerHTML = '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Expecting: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].expected + ' </td> '

            }


            else {
                document.getElementById('expected-or-born' + index).innerHTML += '<th scope="row" class="px-4 py-2   font-bold text-gray-900  text-right"> Birthday: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].birthday.text + '</td>'
                document.getElementById('age-' + index).innerHTML += '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Age: </th> <td class="px-4 py-2 text-left"> ' + calculateWeeksOld(Litters[index].birthday.value) + ' Weeks old</td> '
                // document.getElementById('thumbnail-container-' + index).innerHTML = ' <video controls poster="' + Litters[index].thumbnail.poster + '" class="w-[600px] h-48 md:h-[19rem] lg:h-[19rem] object-cover rounded-lg"> <source src=' + Litters[index].thumbnail.link + ' type="video/mp4"> Your browser does not support the video tag. </video>'


            }

            if (Litters[index].thumbnail.type == 'video') {
                document.getElementById('thumbnail-container-' + index).innerHTML = '<div><video controls poster="' + Litters[index].thumbnail.poster + '" class="w-[600px] h-48 md:h-[19rem] lg:h-[19rem] object-cover rounded-t-lg"> <source src=' + Litters[index].thumbnail.link + ' type="video/mp4"> Your browser does not support the video tag. </video><div class="bg-[#186C91] rounded-b-lg text-center py-3 text-white">' + Litters[index].thumbnail.caption + '</div></div>';
                // document.getElementById('thumbnail-container-' + index).innerHTML = '<video controls poster="' + Litters[index].thumbnail.poster + '" class="w-[600px] h-48 md:h-[19rem] lg:h-[19rem] object-cover rounded-lg"> <source src=' + Litters[index].thumbnail.link + ' type="video/mp4"> Your browser does not support the video tag. </video>'
            ...
            document.getElementById('litter-container').innerHTML += '<div class="shadow m-8 py-10 w-full max-w-screen-xl mx-auto border-[1px] rounded flex flex-wrap justify-center divide-y-2 lg:divide-y-0 lg:divide-x-2"> <div class="w-[100%] md:w-[100%] lg:w-[55%] lg:px-10"> <div class="text-2xl font-extrabold px-10 lg:px-0 text-center lg:text-left ">' + Litters[index].name + '</div>  <table class=" my-4 w-full"> <thead> </thead> <tbody class=""> <tr class="even:bg-white odd:bg-gray-50  border-b border-t "> <th scope="row" class="w-[40%] lg:w-[35%] px-2 py-1 font-bold text-gray-900  text-right"> Litter Type: </th> <td class="px-4 py-2 text-left">' + Litters[index].type + ' </td> </tr> <tr class="even:bg-white odd:bg-gray-50  border-b border-t"> <th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Average Weight: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].avg_weight + ' </td> </tr> <tr class="even:bg-white odd:bg-gray-50  border-b "> <th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Litter Status: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].status.text + '</td> </tr>  <tr id="go-home-row' + index + '" class="even:bg-white odd:bg-gray-50  border-b ">   </tr><tr id="age-' + index + '" class="even:bg-white odd:bg-gray-50  border-b ">   </tr></tbody><tbody id="prices-container' + index + '"></tbody>  </table> <div class="flex justify-center " id="thumbnail-container' + index + '"></div> </div> <div class="w-[100%] lg:w-[45%] "><div class="text-2xl font-extrabold px-10  py-5 text-left "> All Puppies </div> <ul id="multiple-litters' + index + '" class="divide-y divide-gray-200 "></ul> </div> </div>'

            if (Litters[index].status.mode == 'expecting' || Litters[index].status.mode == 'borned') {
                document.getElementById('go-home-row' + index).innerHTML = '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Go-Home Weekend: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].goHome + ' </td>'
            }
            else {
                document.getElementById('go-home-row' + index).innerHTML = '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Birthday: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].birthday.text + '</td>'
                document.getElementById('age-' + index).innerHTML = '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Age: </th> <td class="px-4 py-2 text-left"> ' + calculateWeeksOld(Litters[index].birthday.value) + ' Weeks old</td> '
            }


            for (let innerindex = 0; innerindex < Litters[index].puppies.length; innerindex++) {
                document.getElementById('multiple-litters' + index).innerHTML += '<li onclick="showSubPopup(' + index + ',' + innerindex + ')" class="px-8 mx-2 py-3 sm:py-4 cursor-pointer hover:bg-gray-100 rounded-lg"> <div class="flex items-center"> <div class="flex-shrink-0"> <img class="w-24 h-24 object-cover rounded-full" src="' + Litters[index].puppies[innerindex].image + '" alt="' + Litters[index].puppies[innerindex].name + ' "> </div> <div class="flex-1 min-w-0 ms-4"> <p class="text-sm font-extrabold  text-gray-900 ">' + Litters[index].puppies[innerindex].name + ' (' + Litters[index].puppies[innerindex].gender + ')</p> <p id="availability-badge' + index + innerindex + '" class="text-sm text-gray-500 truncate py-2"></p> <p class="text-sm text-gray-600">' + calculateWeeksOld(Litters[index].puppies[innerindex].birthday.value) + ' Weeks old </p></div> <div> <div class="inline-flex items-center text-sm lg:text-lg font-semibold text-gray-900">  ' + Litters[index].puppies[innerindex].price + ' </div> </div> </div> </li>'

                // Availability Badge !
                if (Litters[index].puppies[innerindex].availablity == "Available") {
                    document.getElementById('availability-badge' + index + innerindex).innerHTML = '    <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-medium bg-teal-100 text-teal-600">' + Litters[index].puppies[innerindex].availablity + '</span>'

                }
                else {
                    document.getElementById('availability-badge' + index + innerindex).innerHTML = '    <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-medium bg-red-100 text-red-600">' + Litters[index].puppies[innerindex].availablity + '</span>'
