    ...
    for (let index = 0; index < Litters.length; index++) {
        if (Litters[index].mode === 0) {
            ...
            if (Litters[index].status.mode == 'expecting') {
                // document.getElementById('expected-or-born').innerHTML += '<div>Ok</div>'
            ...
            if (Litters[index].thumbnail.type == "video") {
                document.getElementById('thumbnail-container').innerHTML = ' <video controls poster="' + Litters[index].thumbnail.poster + '" class="w-full object-cover rounded-lg"> <source src=' + Litters[index].thumbnail.link + ' type="video/mp4"> Your browser does not support the video tag. </video>'
            }
