    ...
    for (let index = 0; index < Litters.length; index++) {
        if (Litters[index].mode === 0) {
            ...
            if (Litters[index].status.mode == 'expecting') {
                ...
                document.getElementById('expected-or-born' + index).innerHTML += '<th scope="row" class="px-4 py-2 font-bold text-gray-900  text-right"> Birthday: </th> <td class="px-4 py-2 text-left"> ' + Litters[index].birthday.text + '<br>(' + calculateWeeksOld(Litters[index].birthday.value) + ' Weeks old)</td> '
            ...
            for (let innerindex = 0; innerindex < Litters[index].puppies.length; innerindex++) {
                document.getElementById('multiple-litters').innerHTML += '<li onclick="showSubPopup(' + index + ',' + innerindex + ')" class="px-8 mx-2 py-3 sm:py-4 cursor-pointer hover:bg-gray-100 rounded-lg"> <div class="flex items-center"> <div class="flex-shrink-0"> <img class="w-24 h-24 rounded-full" src="' + Litters[index].puppies[innerindex].image + '" alt="' + Litters[index].puppies[innerindex].name + ' "> </div> <div class="flex-1 min-w-0 ms-4"> <p class="text-sm font-extrabold  text-gray-900 ">' + Litters[index].puppies[innerindex].name + ' (' + Litters[index].puppies[innerindex].gender + ')</p> <p id="availability-badge' + innerindex + '" class="text-sm text-gray-500 truncate py-2"></p> <p class="text-sm text-gray-600">' + calculateWeeksOld(Litters[index].puppies[innerindex].birthday.value) + ' Weeks old </p></div> <div> <div class="inline-flex items-center text-sm lg:text-lg font-semibold text-gray-900">  ' + Litters[index].puppies[innerindex].price + ' </div> </div> </div> </li>'
